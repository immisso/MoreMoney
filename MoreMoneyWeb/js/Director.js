...
export class Director {
    ...
    coinEvent() {
        for (let i = 0; i < this.dataStore.get('coins').length; i++) {
            this.dataStore.get('coins')[i].time = 0;
        }
    };

    fireEvent() {
        for (let i = 0; i < this.dataStore.get('fires').length; i++) {
            this.dataStore.get('fires').time = 0;
        }
    };
    /**
     * 落物存在两种:
    ...
    check() {
        ...
        const fires_length = fires.length;
